...
def process_version(v):
    ...
    for lib in v["libraries"]:
        if "downloads" in lib:
            if "artifact" in lib["downloads"] and "path" in lib["downloads"]["artifact"]:
            ...
            if "natives" in lib:
                ...
                lib["downloads"]["classifiers"] = {
                    k: v for k, v in lib["downloads"]["classifiers"].items() if k in native_keys
                }
        libs.append(lib)
...
async def main():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        for mc in mcs["versions"]:
            if (
                mc["type"] != "release" and mc["id"] != "1.5"
            ):  # For some reason we have 1.5 at Technic, even tho it's a snapshot
